...
export function Dashboard({ user, onNavigate, onLogout, onOpenNotifications, onRequestMoodPopup }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    // Request mood popup only once per dashboard visit
    if (onRequestMoodPopup && !dashboardVisited) {
  ...
  }, [onRequestMoodPopup, dashboardVisited]);

  // Refetch aura and latest emotion when mood is logged (listen for changes)
  useEffect(() => {
    const handleMoodLogged = async () => {
      console.log('[Dashboard] Mood logged - refreshing aura and emotion');
      try {
        const [latestAura, emotions] = await Promise.all([
          api.getAuraFromLatestEmotion(),
          api.getEmotionLogs({
            start_date: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            end_date: new Date().toISOString().split('T')[0]
          })
        ]);
        setDynamicAura(latestAura);
        if (emotions && emotions.length > 0) {
          setLatestEmotion(emotions[0]);
        }
      } catch (error) {
        console.error('Failed to refresh aura after mood log:', error);
      }
    };

    // Listen for custom event from mood popup
    window.addEventListener('moodLogged', handleMoodLogged);
    
    return () => {
      window.removeEventListener('moodLogged', handleMoodLogged);
    };
  }, []);
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm text-gray-600">Latest Emotion</p>
                <p className="text-2xl font-bold text-gray-800">
                  {(() => {
                    if (!latestEmotion?.emotion) return 'None';
                    const emotion = latestEmotion.emotion;
                    const emotionLabel = emotion.toLowerCase().replace('_', '-');
                    ...
                    const emoji = emojiMap[emotionLabel] || '💫';
                    const displayName = emotion.charAt(0).toUpperCase() + emotion.slice(1).replace('_', ' ');
            ...
            {latestEmotion ? (
              <div className="space-y-1">
                <p className="text-xs text-gray-600">
                  Intensity: <span className="font-semibold">{latestEmotion.intensity || 'N/A'}/10</span>
                </p>
                <p className="text-xs text-gray-600">
                  Logged: <span className="font-semibold">{new Date(latestEmotion.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </p>
              </div>
            ) : (
              <p className="text-xs text-gray-600">No emotion logged today</p>
            )}
          </motion.div>

          {/* Wellness Score - SECOND */}
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="relative grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="space-y-6">
                <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-5 border border-white/50">
                  ...
                  {currentAura.emotionLabel && 
                   currentAura.emotionLabel !== 'No recent mood' && 
                   currentAura.emotionLabel !== 'No mood logged yet' && (
                    <p className="text-xs text-gray-500 mb-3">
                      💫 Based on: <span className="font-semibold">{currentAura.emotionLabel}</span>
                    </p>
                  )}
