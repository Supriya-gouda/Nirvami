...
import { ChevronLeft, CheckCircle2, Clock, Volume2, VolumeX, Sparkles, Youtube, AlertCircle } from 'lucide-react';
...
interface LifestylePracticeProps {
  recommendation: {
    id?: string;
    title: string;
    content: string;
...
export function LifestylePractice({ recommendation, onComplete, onClose }: LifestylePracticeProps) {
  ...
  useEffect(() => {
    ...
    const fetchVideo = async () => {
      try {
        const searchQuery = `${recommendation.title} lifestyle how to follow`;
  ...
  const startPractice = async () => {
    ...
    // Start music
    try {
      await musicManager.current?.init('lifestyle');
      await musicManager.current?.play();
      setIsMusicEnabled(true);
      console.log('🎵 Started lifestyle music');
    } catch (error) {
    ...
    await voiceService.current?.speak(
      `${recommendation.title}. Let's review this lifestyle guidance together.`
    );
  };
  ...
  const toggleMusic = () => {
    ...
    if (newState && hasStarted) {
      musicManager.current?.init('lifestyle').then(() => {
  ...
  if (isCompleting) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-green-50 to-teal-100 flex items-center justify-center z-50">
        <motion.div
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-gray-800">Reminder Acknowledged!</h2>
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-green-50 via-teal-50 to-emerald-50 z-40 overflow-hidden">
      <div className="h-full overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Lifestyle Guidance</p>
        </div>
