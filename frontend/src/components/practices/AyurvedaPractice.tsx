...
import { ChevronLeft, Volume2, VolumeX, CheckCircle2, Clock, Youtube, AlertCircle } from 'lucide-react';
...
interface AyurvedaPracticeProps {
  recommendation: {
    id?: string;
    title: string;
    content: string;
...
export function AyurvedaPractice({ recommendation, onComplete, onClose }: AyurvedaPracticeProps) {
  ...
  useEffect(() => {
    ...
    const fetchVideo = async () => {
      try {
        const searchQuery = `${recommendation.title} ayurveda how to prepare`;
  ...
  const startPractice = async () => {
    ...
    // Start music
    try {
      await musicManager.current?.init('ayurveda');
      await musicManager.current?.play();
      setIsMusicEnabled(true);
      console.log('🎵 Started Ayurveda music');
    } catch (error) {
    ...
    await voiceService.current?.speak(
      `Let's learn about ${recommendation.title}. Take your time to read through the guidance.`
    );
  };
  ...
  const markComplete = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${recommendation.title} has been logged successfully.`,
        duration: 3000,
      });
  ...
  const toggleMusic = () => {
    ...
    if (newState && hasStarted) {
      musicManager.current?.init('ayurveda').then(() => {
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center z-50">
        ...
        >
          ...
          >
            <CheckCircle2 className="w-24 h-24 text-orange-600" />
          </motion.div>
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 z-40 overflow-hidden">
      <div className="h-full overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">Ayurvedic Guidance</p>
        </div>
